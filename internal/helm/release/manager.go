...
func (m manager) RollBack(ctx context.Context, opts ...RollBackOption) error {
	...
	rollback.Force = m.forceRollback

	for _, fn := range opts {
		if err := fn(rollback); err != nil {
			return fmt.Errorf("failed to apply upgrade option: %w", err)
		}
	}
...
func (m manager) UpgradeRelease(ctx context.Context, opts ...UpgradeOption) (*rpb.Release, *rpb.Release, error) {
	...
	if err != nil {
		...
		if upgradedRelease != nil {
			if rollErr := m.RollBack(ctx); rollErr != nil {
				return nil, nil, fmt.Errorf("failed upgrade (%s) and failed rollback: %w", err, rollErr)
			}
		}
